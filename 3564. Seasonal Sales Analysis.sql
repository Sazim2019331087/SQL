...
# Solution:
  --------------------------
WITH reformed_sales AS (
    SELECT
    Extract(Month From S.sale_date) AS sale_month ,
    DECODE(Extract(Month From S.sale_date) , 
    12,'Winter',1,'Winter',2,'Winter',
    3,'Spring',4,'Spring',5,'Spring',
    6,'Summer',7,'Summer',8,'Summer',
    9,'Fall',10,'Fall',11,'Fall'
    ) AS season , 
    S.sale_id, S.product_id,P.category AS category,S.quantity,S.price*S.quantity as revenue
    FROM sales S JOIN products P
    ON S.product_id = P.product_id
)
,   season_wise_sales AS(
...
,   ranked_sale_by_season AS (
        SELECT season,category,total_quantity,total_revenue,
        DENSE_RANK() OVER (
            PARTITION BY season
            ORDER BY total_quantity DESC , total_revenue DESC , category ASC
        ) AS rn
        FROM season_wise_sales 
) 
    SELECT season,category,total_quantity,total_revenue
    FROM ranked_sale_by_season WHERE rn = 1;
